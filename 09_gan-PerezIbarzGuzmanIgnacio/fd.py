...
class Classifier(nn.Module):
    ...
    def pretrained(cls):
        ...
        model.load_state_dict(
            torch.load("classifier.pth", map_location=lambda storage, loc: storage)
        )
        return model
...
class FDScore(nn.Module):
    ...
    def calculate(self, samples):
        ...
        if not np.allclose(minmax, np.array([-1.0, 1.0]), rtol=0.1, atol=0.1):
            print(
                "Warning: the pixel values should be in the range [-1, 1]. The range in the samples is %s."
                % minmax
            )
